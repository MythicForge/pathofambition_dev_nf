...
function getTierColor(spell: { is_cantrip?: boolean; tier?: number }): string {
  if (spell.is_cantrip) return "var(--c-tier-cantrip)";
  const t = spell.tier;
  if (t && t >= 1 && t <= 6) return `var(--c-tier-${t})`;
  return "var(--c-spell)";
}
  ...
  const filtered = useMemo(() => {
    let result = spells.filter((spell) => {
      ...
      if (
        ...
        !(spell.sources ?? []).some((src) => activeSources.has(src))
      )
        return false;
      if (search.trim()) {
                ...
                {SORT_OPTIONS.map((opt) => (
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor:
                        sort === opt
                          ? "rgb(var(--c-spell-rgb) / 0.10)"
                          : "transparent",
                      color:
                        sort === opt ? "var(--c-spell)" : "var(--text-primary)",
                      fontFamily: "var(--font-mono)",
                      fontSize: "0.72rem",
                      cursor: "pointer",
                    }}
        ...
        >
          ...
          {schools.map(([school, count]) => (
            <FilterPill
              ...
              onClick={() =>
                setActiveSchool(activeSchool === school ? null : school)
              }
            />
          ))}
        </div>
