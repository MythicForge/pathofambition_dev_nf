...
const CAT_TYPE: Record<ProfCategory, 'action' | 'spell' | 'feat'> = {
  combat: 'action',
  magic: 'spell',
  stealth: 'feat',
};
...
export default async function ProfessionDetailPage({ params }: Props) {
  ...
  const statTiles = [
    { label: 'Starting Vitality', value: prof.starting_vitality || '—', icon: ICON_VITALITY },
    { label: 'Favored', value: prof.favored_attributes_raw || '—', icon: ICON_FAVORED },
    { label: 'Vitality / Tier', value: prof.vitality_gained_per_tier || '—', icon: ICON_GROWTH },
    ...(prof.wound_bonus_per_tier ? [{ label: 'Wound Bonus / Tier', value: `+${prof.wound_bonus_per_tier}`, icon: ICON_SHIELD }] : []),
  ];
  ...
  return (
    <div style={{ maxWidth: '960px' }}>
      ...
      <nav aria-label="Breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '20px', fontFamily: 'var(--font-mono)', fontSize: '0.7rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
        <Link href="/professions" style={{ color: catColor, textDecoration: 'none' }}>Professions</Link>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {prof.starting_vitality && (
          <span style={chipStyle}>
            <span style={{ color: 'var(--text-tertiary)', letterSpacing: '0.1em', textTransform: 'uppercase', fontSize: '0.65rem' }}>Vitality</span>
            <span style={{ color: catColor }}>{prof.starting_vitality}</span>
          </span>
        )}
        {prof.favored_attributes_raw && (
          <span style={chipStyle}>
            <span style={{ color: 'var(--text-tertiary)', letterSpacing: '0.1em', textTransform: 'uppercase', fontSize: '0.65rem' }}>Favored</span>
            <span style={{ color: catColor }}>{prof.favored_attributes_raw}</span>
          </span>
        )}
        {prof.path_options.length > 0 && (
          <span style={chipStyle}>
            <span style={{ color: 'var(--text-tertiary)', letterSpacing: '0.1em', textTransform: 'uppercase', fontSize: '0.65rem' }}>Paths</span>
            <span style={{ color: 'var(--text-primary)' }}>{prof.path_options.join(' · ')}</span>
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: '28px', marginBottom: '36px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {statTiles.map((t) => (
            <StatTile key={t.label} label={t.label} value={t.value} icon={t.icon} category={catType} />
          ))}
          ...
          {(prof.proficiencies.vitals_skills.length > 0 ||
            ...
            <div style={{ paddingTop: '4px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9.5px', letterSpacing: '1.8px', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: '10px' }}>
                Proficiencies
              </div>
              {prof.proficiencies.vitals_skills.length > 0 && (
                <div style={{ marginBottom: '8px' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8.5px', letterSpacing: '1.2px', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: '5px' }}>Skills</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {prof.proficiencies.vitals_skills.map((s) => <CategoryTag key={s} label={s} category={catType} />)}
                  </div>
                </div>
              )}
              {prof.proficiencies.armaments.length > 0 && !(prof.proficiencies.armaments.length === 1 && prof.proficiencies.armaments[0] === '-') && (
                <div style={{ marginBottom: '8px' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8.5px', letterSpacing: '1.2px', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: '5px' }}>Armaments</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {prof.proficiencies.armaments.map((a) => <CategoryTag key={a} label={a} category={catType} />)}
                  </div>
                </div>
              )}
              {prof.proficiencies.protection.length > 0 && !(prof.proficiencies.protection.length === 1 && prof.proficiencies.protection[0] === '-') && (
                <div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8.5px', letterSpacing: '1.2px', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: '5px' }}>Protection</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {prof.proficiencies.protection.map((p) => <CategoryTag key={p} label={p} category={catType} />)}
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: 0 }}>
          ...
          <div style={{
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9.5px', letterSpacing: '1.8px', textTransform: 'uppercase', color: catColor, marginBottom: '10px' }}>
              Overview
            </div>
                  ...
                  <div key={feat.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: feat.description_markdown ? '8px' : 0 }}>
                      ...
                      {feat.trait_label && feat.trait_label !== '-' && (
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginLeft: 'auto' }}>{feat.trait_label}</span>
                      )}
                    </div>
            ...
            return (
              <div key={pathName} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                  <span style={{ width: '3px', height: '18px', borderRadius: '2px', backgroundColor: catColor, flexShrink: 0 }} />
                  <h2 style={{ fontFamily: 'var(--font-heading)', fontStyle: 'italic', fontWeight: 500, fontSize: '1.2rem', color: 'var(--text-primary)', margin: 0 }}>
                    {pathName}
                  </h2>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9.5px', letterSpacing: '1.2px', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginLeft: 'auto' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {featsForPath.map((feat) => (
                    <div key={feat.id} style={{
                      ...
                      <span style={{ width: '5px', height: '5px', borderRadius: '50%', backgroundColor: catColor, flexShrink: 0, marginTop: '7px' }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: feat.description_markdown ? '4px' : 0 }}>
                          ...
                          {feat.cost && (
                            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.5px', color: catColor, flexShrink: 0, marginLeft: 'auto' }}>{feat.cost}</span>
                          )}
                        </div>
                        {feat.required && (
                          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-tertiary)', marginBottom: '4px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
            {related.map((p) => (
              ...
              >
                ...
                <span style={{ color: catColor, fontSize: '14px', opacity: 0.8 }}>→</span>
              </Link>
            ))}
          </div>
        </div>
